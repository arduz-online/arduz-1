        ...
        With AdapInfo
            If (.AdapterType = MIB_IF_TYPE_ETHERNET) Then
                If sz2string(.Description) Like "*Hamachi*" Then
                        hIP = sz2string(.IpAddressList.IpAddress.IPadrString) 'MAC2String(.MACaddress) & vbCrLf '&  & vbCrLf
                        frmMain.hamaa.Enabled = True
                        frmMain.hamaa.value = vbChecked
                        frmMain.hamaa.ToolTipText = hIP & " - Red oficial: Arduz, asegurate de estar conectado."
                End If
            End If
        End With
