...
'***************************************************
    ...
    With charlist(CharIndex)
        ...
        .muerto = (IIf(headIndex = CASPER_HEAD Or headIndex = CASPER_HEAD1, True, False))
...
On Error GoTo ErrHandler
    ...
    For i = 0 To UBound(creatures())
        ...
        'Call WriteTrain(lstCriaturas.listIndex + 1)//asdasdasdasdasdasd
    Next i
...
Loop
        ...
        Ekipos(eKip.eCUI).Nombre = "Equipo azul"
        Ekipos(eKip.ePK).num = countekip(eKip.ePK)
        Ekipos(eKip.ePK).color = RGB(250, 0, 0)
        Ekipos(eKip.ePK).Nombre = "Equipo rojo"
        Ekipos(eKip.eNone).num = countekip(eKip.eNone)
        Ekipos(eKip.eNone).color = RGB(127, 127, 127)
        Ekipos(eKip.eNone).Nombre = "Espectadores"
...
Public Sub WriteChangelevel(ByVal map As String)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "PartyKick" message to the outgoing data buffer
