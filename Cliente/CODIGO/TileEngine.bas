...
'**************************************************************
    ...
    Exit Sub
eroraro:
    Call SurfaceDB.Initialize(DirectDraw, False, DirGraficos, ClientSetup.byMemory) 'We are done!
...
'***************************************************
    ...
    If EngineRun Then
        ...
        yy = 330
        If IScombate And (Not frmMain.SendTxt.Visible) Then
            Dim i As Integer
            'totalx = totalxs + IIf(Ekipos(eKip.eCUI).num > 0, 1, 0) + IIf(Ekipos(eKip.eNone).num > 0, 1, 0) + IIf(Ekipos(eKip.ePK).num > 0, 1, 0)
            'BackBufferSurface.SetFillColor RGB(5, 40, 20)
            'BackBufferSurface.SetForeColor RGB(0, 0, 0)
            'BackBufferSurface.SetFillStyle 0
            'BackBufferSurface.DrawRoundedBox 290, yy + 5, 770, (totalx + 1) * 15 + yy + 5, 10, 10
            Call RenderText2(300, yy, CStr("Ping: " & pinga & " ms."), vbYellow, frmMain.font)
            For isa = 0 To 2
            i = Abs(isa - 2)
                If Ekipos(i).num > 0 Then
                    yy = yy + 15
                    Call RenderText2(300, yy, Ekipos(i).Nombre, Ekipos(i).color, frmMain.font)
                    Call RenderText2(600, yy, "Frags", Ekipos(i).color, frmMain.font)
                    Call RenderText2(650, yy, "Muertes", Ekipos(i).color, frmMain.font)
                    Call RenderText2(710, yy, "Puntos", Ekipos(i).color, frmMain.font)
                    For xx = 1 To Ekipos(i).num
                        If Ekipos(i).personajes(xx) > -1 Then
                            yy = yy + 15

                            'BackBufferSurface.DrawBox
                            Call RenderText2(310, yy, pjs(Ekipos(i).personajes(xx)).Nick & IIf(pjs(Ekipos(i).personajes(xx)).bot = False, "", "             <BOT>") & IIf(Len(pjs(Ekipos(i).personajes(xx)).clan) > 0, " <" & pjs(Ekipos(i).personajes(xx)).clan & ">", ""), IIf(pjs(Ekipos(i).personajes(xx)).Nick = frmMain.Label8, vbWhite, IIf(pjs(Ekipos(i).personajes(xx)).gm = False, Ekipos(i).color, RGB(0, 255, 100))), frmMain.font)
                            Call RenderText2(600, yy, CStr(pjs(Ekipos(i).personajes(xx)).frags), Ekipos(i).color, frmMain.font)
                            Call RenderText2(650, yy, CStr(pjs(Ekipos(i).personajes(xx)).muertes), Ekipos(i).color, frmMain.font)
                            Call RenderText2(710, yy, CStr(pjs(Ekipos(i).personajes(xx)).Puntos), Ekipos(i).color, frmMain.font)
                        End If
                    Next xx
                End If
            Next isa
...
Public Sub RenderText(ByVal lngXPos As Integer, ByVal lngYPos As Integer, ByRef strText As String, ByVal lngColor As Long, ByRef font As StdFont)
    If strText <> "" Then
        ...
        Call BackBufferSurface.DrawText(lngXPos + 1, lngYPos + 1, strText, False) '-2 -1
...
Public Sub RenderText1(ByVal lngXPos As Integer, ByVal lngYPos As Integer, ByRef strText As String, ByVal lngColor As Long, ByRef font As StdFont)
    If strText <> "" Then
        Call BackBufferSurface.SetForeColor(vbBlack)
        Call BackBufferSurface.SetFont(font)
        Call BackBufferSurface.DrawText(lngXPos - 1, lngYPos - 1, strText, False)
        Call BackBufferSurface.DrawText(lngXPos - 1, lngYPos, strText, False)
        Call BackBufferSurface.DrawText(lngXPos + 1, lngYPos, strText, False)
        Call BackBufferSurface.DrawText(lngXPos + 1, lngYPos + 1, strText, False)
        Call BackBufferSurface.DrawText(lngXPos + 1, lngYPos - 1, strText, False)
        Call BackBufferSurface.DrawText(lngXPos - 1, lngYPos + 1, strText, False)
        Call BackBufferSurface.DrawText(lngXPos, lngYPos + 1, strText, False)
        Call BackBufferSurface.DrawText(lngXPos, lngYPos - 1, strText, False)
    ...
    End If
End Sub
Public Sub RenderText2(ByVal lngXPos As Integer, ByVal lngYPos As Integer, ByRef strText As String, ByVal lngColor As Long, ByRef font As StdFont)
    If strText <> "" Then
        Call BackBufferSurface.SetForeColor(vbBlack)
        Call BackBufferSurface.SetFont(font)
        Call BackBufferSurface.DrawText(lngXPos - 1, lngYPos, strText, False)
                ...
                If .Head.Head(.Heading).GrhIndex Then
                    ...
                    If LenB(.Nombre) > 0 Then
                        If Nombres Then 'And Abs(MouseTileX - .Pos.x) < 2 And (Abs(MouseTileY - .Pos.y)) < 2 Then
                            ...
                            Else
                                color = RGB(128, 128, 128) 'RGB(ColoresPJ(.priv).r, ColoresPJ(.priv).g, ColoresPJ(.priv).b)
                            End If
